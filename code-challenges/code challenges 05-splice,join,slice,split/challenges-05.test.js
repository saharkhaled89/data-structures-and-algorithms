...
const wordsToCharList = (arr) => {
  // Solution code he  return arr.split('');
  return arr.split('');
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  const lengtharray = recipe.ingredients.length;
  for (let i = 0; i < lengtharray; i++) {
    const firstSpace = recipe.ingredients[i].indexOf(' ') + 1;
    result.push(recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ', firstSpace)+1));
  }
...
const splitFoods = (recipe) => {
  let result = [];
  // Solution code here...
  const lengthOfarray = recipe.ingredients.length;
