...
const getHouses = (arr) => {
  ...
  for (let index = 0; index < arr.length; index++) {
    houses.push(Object.values(arr[index])[3]);
  }
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  for (let index = 0; index < arr.length; index++) {
    let tempValues = Object.values(arr[index]);
    if(tempValues[0] === character){
      let tempBool = (tempValues[2].length === 0) ? false : true;
      return tempBool;
    }
  }
  return false;
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.entries(arr).forEach(item =>
     { item.forEach(person => { if(person.name === character) { if( children = person.children.length > 0) { children==true;} else{ children==false; } } }) })
  return children;};
